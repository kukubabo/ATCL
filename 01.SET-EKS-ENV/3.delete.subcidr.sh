...
################################################
kubectl delete eniconfig ap-southeast-2a
#kubectl delete eniconfig ap-southeast-2b
kubectl delete eniconfig ap-southeast-2c
...
################################################
##### disassociate secondary cidr block
################################################
#ASSOCIATE_ID=`aws ec2 describe-vpcs         --filters Name=vpc-id,Values=$VPC_ID | jq -r '.Vpcs[].CidrBlockAssociationSet[] | select(.CidrBlock | startswith("128")) | .AssociationId'`
ASSOCIATE_ID=`aws ec2 describe-vpcs --filters Name=vpc-id,Values=$VPC_ID | jq -r '.Vpcs[].CidrBlockAssociationSet[] | select(.CidrBlockState[]=="associated") | select(.CidrBlock | startswith("128")) | .AssociationId'`
aws ec2 disassociate-vpc-cidr-block --association-id $ASSOCIATE_ID
