...
export VPC_ID=$(eksctl utils describe-stacks --region=ap-southeast-2 --cluster=${EKS_CLUSTER} | grep OutputValue | grep vpc | cut -d"\"" -f2)
export SUB_CIDR="128.0.0.0/16"

aws ec2 associate-vpc-cidr-block --vpc-id ${VPC_ID} --cidr-block ${SUB_CIDR}

export AZ1=ap-southeast-2a
#export AZ2=ap-southeast-2b
export AZ3=ap-southeast-2c
...
export RTB_ID_A=$(aws ec2 describe-route-tables --filters Name=vpc-id,Values=$VPC_ID | jq -r '.RouteTables[] | select(.Tags[].Value | startswith("PrivateRouteTableAPSOUTHEAST2A")) | .RouteTableId')
#export RTB_ID_B=$(aws ec2 describe-route-tables --filters Name=vpc-id,Values=$VPC_ID | jq -r '.RouteTables[] | select(.Tags[].Value | startswith("PrivateRouteTableAPSOUTHEAST2B")) | .RouteTableId')
export RTB_ID_C=$(aws ec2 describe-route-tables --filters Name=vpc-id,Values=$VPC_ID | jq -r '.RouteTables[] | select(.Tags[].Value | startswith("PrivateRouteTableAPSOUTHEAST2C")) | .RouteTableId')
